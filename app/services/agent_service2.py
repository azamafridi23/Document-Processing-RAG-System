...
class DocumentAgent:
    ...
    def query_rewriter(self, query: str) -> str:
        """
        Rewrites the user query to optimize it for vector search.
        This can be as simple or as complex as needed (e.g., keyword extraction, paraphrasing, etc.).
        For now, we'll just return the query as-is, but this is where you would add logic.
        """
        # TODO: Add advanced rewriting logic if needed
        ...
        retrieved_query = retrieval_query.content

        print(f"Rewritten Query: {retrieved_query}")
    ...
    def semantic_search(self, user_query: str):
        ...
        """
        rewritten_query = self.query_rewriter(user_query) # Pre-retrieval Query Rewriting
        file_level_docs = self._get_docs_from_relevant_files(rewritten_query)
        
        retriever = self.vector_manager.get_retriever(
            collection_name=VECTORSTORE_COLLECTION_NAME, async_mode=False, k=20
        )
        # Use OpenAI's GPT-4o for reranking
        compressor = RankLLMRerank(top_n=10, model="gpt", gpt_model="gpt-4o")
        compression_retriever = ContextualCompressionRetriever(
            base_compressor=compressor, base_retriever=retriever
        )
        semantic_docs = compression_retriever.invoke(rewritten_query)
...
agent = DocumentAgent()
    ...
    print("\n--- Testing Agent ---")
    test_query = "What is the company philosophy?"
